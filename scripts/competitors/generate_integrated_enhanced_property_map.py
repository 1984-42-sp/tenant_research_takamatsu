...
competitors.forEach((c) => {
  ...
  const popupHtml = `
    <div style="font-size:13px; line-height:1.6;">
      ...
      ジャンル：${safe(c.genre)}<br>
      住所：${safe(c.address)}<br>
      評価：${safe(c.rating)}<br>
      口コミ数：${safe(c.review_count)}<br>
      ${
        c.url
          ? `<a href="${c.url}" target="_blank" rel="noopener">食べログページ</a>`
          : ""
      }
    </div>
  `;
...
competitorLayer.addTo(__MAP_NAME__);
